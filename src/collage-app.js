...
const editorControls=document.createElement('div');editorControls.className='editor-controls';editorControls.append(document.querySelector('.mode-switch'),$('canvas-tools'));$('canvas-wrap').before(editorControls);
...
function draw(){
 ...
 ox.fillStyle='#FFFFFF';for(const h of (frameMode?frameHandles():photoHandles(b))){ox.beginPath();if(frameMode&&frameTool==='slant'){ox.moveTo(h.x,h.y-8*scale);ox.lineTo(h.x+8*scale,h.y);ox.lineTo(h.x,h.y+8*scale);ox.lineTo(h.x-8*scale,h.y);ox.closePath()}else ox.arc(h.x,h.y,6*scale,0,Math.PI*2);ox.fill();ox.stroke()}
...
function controls(){
 $('frame-proportion').setAttribute('aria-pressed',String(frameTool==='proportion'));$('frame-slant').setAttribute('aria-pressed',String(frameTool==='slant'));$('frame-slant').disabled=state.slots.length<2;$('slant-straight').hidden=frameTool!=='slant';$('frame-reset').hidden=frameTool==='slant';$('make-hero').hidden=frameTool==='slant';document.querySelector('.slant-panel').hidden=true;
...
document.querySelector('.stage').addEventListener('pointerdown',e=>{if(!ready||gesture||!state.slots.length||e.button!==0)return;if(e.target.matches('.stage,.stage-caption,.stage-top,.stage-top span,#status')){finishZoom();frameMode=true;refresh()}});
...
$('canvas-zoom-in').onclick=()=>$('zoom-in').click();$('canvas-zoom-out').onclick=()=>$('zoom-out').click();$('canvas-center').onclick=()=>$('center').click();
 for(const tool of ['proportion','slant'])$('frame-'+tool).onclick=()=>{if(gesture)return;finishZoom();frameTool=tool;frameMode=true;refresh()};$('slant-straight').onclick=()=>$('slant-reset').click();
...
const icons=JSON.parse($('icon-data').textContent);for(const [id,name,label] of [['photo-mode','photo','相片'],['frame-mode','frame','相框'],['canvas-zoom-out','minus',''],['canvas-zoom-in','plus',''],['canvas-center','focus-centered',''],['frame-reset','rotate-2',''],['make-hero','layout-dashboard',''],['slant-straight','frame',''],['undo','arrow-back-up','復原'],['redo','arrow-forward-up','重做'],['export-top','download','下載 JPG'],['export-bottom','download','下載 JPG'],['mobile-canvas','frame','畫布'],['mobile-layout','layout-grid','排版'],['mobile-photo','photo','相片'],['mobile-brand','brand-sketch','品牌']]){const holder=document.createElement('template');holder.innerHTML=icons[name];const svg=holder.content.firstElementChild;svg.classList.add('ui-icon');svg.setAttribute('aria-hidden','true');svg.setAttribute('focusable','false');$(id).replaceChildren(svg,document.createTextNode(label))}
