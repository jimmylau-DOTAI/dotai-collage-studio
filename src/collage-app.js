...
function controls(){
 selected=Math.max(0,Math.min(selected,state.slots.length-1));renderLayouts();$('photo-count').textContent='目前 '+state.slots.length+' / 9 張';$('add-files').disabled=state.slots.length>=9;
 const tilted=!!(state.slant.x||state.slant.y);$('slant-toggle').setAttribute('aria-pressed',String(tilted));$('slant-toggle').textContent=tilted?'關閉斜切':'啟用斜切';$('slant-toggle').disabled=state.slots.length===1;
 ['x','y'].forEach(axis=>{$('slant-'+axis).value=Math.round(state.slant[axis]*100);$('slant-'+axis+'-value').value=Math.round(state.slant[axis]*100);$('slant-'+axis).disabled=state.slots.length===1});$('slant-reset').disabled=!tilted;
...
function thumbs(){const out=$('photos');out.replaceChildren();state.slots.forEach((s,i)=>{const card=document.createElement('div'),e=document.createElement('button'),image=document.createElement('img'),label=document.createElement('span'),remove=document.createElement('button');card.className='photo-card';e.className='photo';e.setAttribute('aria-pressed',String(i===selected));e.title='按一下選取；雙擊換相片';image.src=thumbnail(images[s.photo]);image.alt='相片 '+(i+1)+' 預覽';label.textContent='相片 '+(i+1);e.append(image,label);e.onclick=()=>{selected=i;syncPhotoSelection();controls();draw()};e.ondblclick=()=>pickReplacement(i);remove.className='photo-remove';remove.textContent='移除';remove.setAttribute('aria-label','移除相片 '+(i+1));remove.disabled=state.slots.length===1;remove.onclick=()=>{if(state.slots.length===1)return;batchToken++;rememberAnd(()=>{state.slots=state.slots.filter((_,index)=>index!==i);if(selected>i)selected--;reflow()});say('已移除相片；可按復原還原')};card.append(e,remove);out.append(card)})}
function reflow(){state.layout=C.layoutsFor(state.slots.length)[0].id;state.cut={top:.5,bottom:.5,left:.5,right:.5};state.slots.forEach(s=>{delete s.frame;delete s.mask});if(state.slots.length===1)state.slant={x:0,y:0}}
...
canvas.addEventListener('pointerdown',e=>{if(!ready||gesture||e.button!==0)return;finishZoom();const p=coords(e),rects=C.frameBoxes(state);let i=rects.length-1;while(i>=0&&!C.hit(rects[i],p))i--;if(i<0)return;selected=i;const b=rects[i],slot=state.slots[i],g=C.geometry(images[slot.photo],b,slot);gesture={id:e.pointerId,index:i,start:p,b,slot:{x:slot.x,y:slot.y},g,before:clone(state),moved:false};canvas.setPointerCapture(e.pointerId);refresh()});
canvas.addEventListener('pointermove',e=>{if(!gesture||gesture.id!==e.pointerId)return;const p=coords(e),dx=p.x-gesture.start.x,dy=p.y-gesture.start.y;if(!gesture.moved&&Math.abs(dx)+Math.abs(dy)<2)return;gesture.moved=true;
 ...
 if(gesture.g.overflowX)s0.x=C.clamp(gesture.slot.x-dx/gesture.g.overflowX);if(gesture.g.overflowY)s0.y=C.clamp(gesture.slot.y-dy/gesture.g.overflowY);
 refresh()});
...
async function uploadCollection(e,append){
 ...
 try{
  ...
  rememberAnd(()=>{const start=images.length,oldCount=state.slots.length;images.push(...assets.map(a=>a.image));const added=assets.map((_,i)=>({photo:start+i,x:.5,y:.5,zoom:1}));state.slots=append?[...state.slots,...added]:added;reflow();selected=append?oldCount:0});
...
try{images.push(...await Promise.all(photos.map(p=>load(p.src))));ready=true;refresh();$('initial-preview').hidden=true;$('export-top').disabled=false;$('export-bottom').disabled=false;say('四張示範圖已就位 · 相框固定，拖圖片調構圖') }catch(err){say(err.message,true)}
})();
