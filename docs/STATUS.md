...
- This is final **automated** QA for this fix, not browser/touch acceptance. Original decoding still requires memory; very large sources can exceed low-memory devices. Public source release was approved separately on 2026-09-20; a hosted deployment remains out of scope. Download current artwork before refreshing the same `dist/index.html`; editable photo projects are not saved by the brand kit.
