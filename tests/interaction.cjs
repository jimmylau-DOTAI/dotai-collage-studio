...
(async()=>{
  ...
  try{
    ...
    const beforeResize=snap(),frame=beforeResize.slots[0].frame;
    ...
    const resized=snap();assert.equal(resized.slots[0].frame.x,frame.x);assert.equal(resized.slots[0].frame.y,frame.y);assert(resized.slots[0].frame.w>frame.w&&resized.slots[0].frame.h>frame.h,'resize changes exact dimensions');
    ...
    el('reset').click();const straight=snap();assert.equal(straight.layout,'grid');pointer('pointerdown',540,18);pointer('pointermove',600,18);pointer('pointerup',600,18);const cut=snap();assert.equal(cut.layout,'cut-grid');assert.notEqual(cut.cut.top,straight.cut.top);el('undo').click();assert.deepEqual(snap(),straight,'cut conversion is undoable exactly');assert.equal(editor.errors.length,0);
    console.log('PASS: exact frame move/resize, cancel+redo, slider history/no-op, and shared grid cuts.');
