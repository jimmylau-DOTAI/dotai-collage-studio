...
(async()=>{
  ...
  try{
    ...
    pointer('pointerdown',frame.x+frame.w-50,frame.y+frame.h-50);pointer('pointermove',frame.x+frame.w+50,frame.y+frame.h+40);pointer('pointerup',frame.x+frame.w+50,frame.y+frame.h+40);
    ...
    el('reset').click();el('show-demo').click();const straight=snap();assert.equal(straight.layout,'grid');el('slant-toggle').click();const cut=snap();assert.equal(cut.layout,'grid');assert.notEqual(cut.slant.x,straight.slant.x);el('undo').click();assert.deepEqual(snap(),straight,'slant conversion is undoable exactly');assert.equal(editor.errors.length,0);
