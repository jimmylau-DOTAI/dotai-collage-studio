...
let s=C.defaults();assert.deepEqual(C.size(s),{w:1080,h:1080});const portrait={...C.defaults(),ratio:'4:5'};assert.deepEqual(C.size(portrait),{w:1080,h:1350});
for(const box of C.frameBoxes(portrait)){assert(box.x>=0&&box.y>=0);assert(box.x+box.w<=1080.001&&box.y+box.h<=1350.001);}
assert(C.layouts.length>=24,'Reference library covers distinct four-photo structures');
for(const ratio of ['1:1','4:5'])for(const l of C.layouts){s=C.defaults();s.ratio=ratio;s.layout=l.id;const {w,h}=C.size(s);const b=C.frameBoxes(s);assert.equal(b.length,4);for(const r of b)assert(r.x>=0&&r.y>=0&&r.w>0&&r.h>0&&r.x+r.w<=w+.01&&r.y+r.h<=h+.01);}
